...
On a phone the whole shell mirrors: the back arrow and the chevrons point right-to-left, a tablet's rail stands on the right and its note sheet slides in from the left (the Classic layout's drawer, likewise, from the right). Every dialog (settings, banner picker, moderation, login, confirm) is laid out right-to-left, and the blog is mirrored the same way.
...
The way back is always on screen. If you switched to Arabic by mistake, or someone handed you a laptop whose interface you cannot read, you do not need to find Settings in a language you cannot read. Any one of these will do:

- **The language key on the status bar.** At the end of the bar along the bottom of the window, beside the Reading and Vim switches, there is a small bordered key with a globe. It is labelled with the *other* language's own name: `ع` while the interface is English, `EN` while it is Arabic. Click it and the interface switches: the words, the direction, the side the panes sit on. Click it again and you are back. Hovering over it shows "Switch to Arabic · التبديل إلى العربية" (or the English equivalent), so the tooltip can be read in either language. It is not the note's `EN ⇄ ع` twin pill: that one turns a *note* over to its other face ([linguistic twins](templates-and-notes.md#linguistic-twins)), and this one switches everything *around* the note. Switching never moves your caret or changes which note is open.
- **On a phone**, the same key is at the top right of the **More** tab and of **Settings**, and the note screen's ⋯ sheet has a row for it under **Actions**, written in both languages.
...
Each of these writes the same preference as **Settings → This device → Editor language**, and that row shows the result. Two presses always come back to *Follow site* rather than leaving the site's own language pinned. The three palette commands `Editor language: English`, `Editor language: العربية` and `Editor language: follow the site` are still there for choosing one of the three states directly.
...
**The same post in both languages.** Under `follow`, a reader only ever meets one of the two, which is the point — but it means a reader landing on the English post from a search engine, tapping ع, would previously be left on an address their new language no longer lists. Declare the two files each other's [linguistic twin](templates-and-notes.md#linguistic-twins) and ع/EN takes them to the *other face of the same post* instead of to the home page. Both article pages then carry `<link rel="alternate" hreflang="…">` for both faces, and both appear in the sitemap as alternates of each other, so a crawler files them as one article in two languages rather than as duplicates. A `[[wikilink]]` that names a face this reader cannot see resolves to the face they can, on the public site only — in your editor a link always lands where you pointed it. Under `off` both faces are listed, because under `off` they are simply two published files and saying otherwise would be a lie.
