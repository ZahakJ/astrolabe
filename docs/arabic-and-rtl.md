...
The three share one table (`shared/tashkeel.ts`): the strip removes exactly the code points the palette can write, U+064B–U+0652, U+0670 and U+0640, and no more. The Qur'anic pause marks and small high letters are not in it, on purpose: a strip that reached past what the palette writes would take marks a careful typist never asked it to touch.
