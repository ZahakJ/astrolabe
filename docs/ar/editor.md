...
- **تحديد يفهم ما تحته.** النقر المزدوج يأخذ الكلمة التي تحت المؤشر، ويعدّ الحروف بعناقيدها، فتبقى الحركات العربية وفاصل الكلمات الفارسي (ZWNJ) داخل الكلمة ولا يقطعانها. وإذا نقرت نقرًا مزدوجًا على شيء معروض أخذته كله: رابط ويكي، أو `#وسم`، أو مقطع `$رياضيات$`، أو شريحة كود. وداخل سياج الكود يأخذ النقر المزدوج المعرِّف كاملًا، حتى `$jquery` و`snake_case_name`. النقر الثلاثي يأخذ الفقرة، والسحب يمدّ التحديد حرفًا حرفًا، والنقر مع Shift يمدّه من حيث كنت. تختبر البوابة `npm run check-caret` هذا كله في الواجهة الإنجليزية والعربية.
- **بطاقة الخصائص تُحرَّر في مكانها.** ما دام مؤشرك خارج المقدمة تنطوي إلى بطاقة أنيقة من مفاتيح وقيم، فيها وسوم تُنقر، وتحرّرها هناك مباشرة: انقر قيمة لتكتب فوقها، وعلّم مربعًا لتبدّل بين `true` و`false`، واختر تاريخًا من تقويم، وأضف قيم القوائم أو أزلها شرائحَ، وأضف خاصية، أو أزل خاصية بالعلامة × في آخر صفها. يعرض **إضافة خاصية** كل مفتاح يفهمه التطبيق (`title` و`tags` و`aliases` و`banner` و`date` و`publish` و`description` و`dir` و`align` و`numbered` و`icon` و`language` و`cssclasses`) مع سطر يشرح ما يفعله كل منها. اكتب لترشّح القائمة، وتنقّل بـ↑/↓ ثم اضغط Enter لتأخذ واحدًا، أو اكتب أي مفتاح من عندك. يُكتب `tags` و`aliases` قوائم؛ فالقيم المفصولة بفواصل تصير عناصر. والملاحظة التي لا خصائص لها بعدُ تحمل البطاقة أيضًا: سطر واحد فيه *إضافة خاصية* و*تعيين الغلاف…*، فتبدأ كل ملاحظة من المكان نفسه (يوقف ذلك الخيار *بطاقة الخصائص في الملاحظات الفارغة* في الإعدادات ← المظهر واللغة).
  كل تعديل من هذه **جراحي على مستوى البايت**: يبقى أسلوب علامات الاقتباس عندك، وتعليقاتك، وترتيب مفاتيحك، وكل سطر لم تمسّه، كما كان بالضبط. وحذف آخر خاصية يحذف معها سياجَي `---` بدل أن يترك خطًّا فارغًا. أما مفاتيح الآلة (`id` و`uuid` و`dg-*`) فتبقى للقراءة فقط، ولـ`publish:` مفتاحه الخاص في شريط الحالة. ويعمل الأمر نفسه في ملاحظة `.tex`، حيث تسكن الخصائص في كتلة تعليق تبدأ بـ`%---`.
...
- **الصق المرفقات أو أسقطها.** الصورة التي في حافظتك، أو أي ملف مقبول تسحبه من مدير الملفات (PDF وصوت وفيديو أيضًا)، تُرفع وتحطّ بصيغة `![[name.png]]` عند المؤشر، ويحجز مكانها عنصر نائب مكتوب عليه «جارٍ الرفع…» حتى يصل الملف. وأين يحطّ ما تلصقه أو تسقطه في الملاحظة [إعداد](configuration.md#attachments) تختاره. وتستطيع أيضًا أن تسقط الملفات مباشرة على شجرة الشريط الجانبي: على صف مجلد، أو صف ملاحظة، أو خلفية الشجرة نفسها. والإسقاط على الشجرة تصنيف: يحطّ الملف في المجلد الذي قصدته، مهما قال الإعداد. والنوع الذي سيرفضه الخادم يُرفض قبل أن يبدأ الرفع لا بعده، والرسالة التي تخبرك بنتيجة الإسقاط تحمل زر تراجع.
...
- **معاينة عند التحويم.** ضع المؤشر على `[[رابط ويكي]]` فتظهر بطاقة عائمة فيها مطلع الملاحظة المقصودة معروضًا (و`[[ملاحظة#عنوان]]` يعاين من ذلك العنوان). ومراجع الحواشي تعاين تعريفها.
...
- **عرض المخطط.** خريطة لملاحظاتك: نقطة لكل ملاحظة، وخط لكل رابط، ترسمها محاكاة قوى كتبناها بأيدينا على لوحة رسم. اسحب النقاط، وحوّم على واحدة لتضيء جيرانها، وانقر لتفتح. يُفتح المخطط [تبويبًا](workspace.md#panes) في اللوحة المركَّز عليها (`Ctrl/Cmd G`)، فيجلس المخطط والملاحظة جنبًا إلى جنب أو يتناوبان. ويفتح زر المنزلقات إعداداته: لوّن الملاحظات بحسب **المجلد** (بعمق مستوى أو مستويين) أو بحسب **الوسم**، مع مفتاح ألوان تستطيع فيه أن تعيد تلوين كل مجموعة أو تخفيها؛ وبحث يضيء الملاحظات المطابقة؛ ومرشحات للملاحظات اليتيمة ولحدّ أدنى من الروابط؛ والقوى الثلاث (الانتشار، وطول الرابط، والجذب إلى المركز)؛ وحجم النقطة، وشفافية الروابط، ومستوى التكبير الذي تظهر عنده الأسماء، وهالة. ولوحة الإعدادات نفسها تضعها حيث تشاء: اسحبها من عنوانها إلى أي مكان فوق المخطط، واسحب زاويتها لتجعلها قصيرة أو ضيقة كما تحب، ويعيدها *إعادة الضبط* إلى مكانها. يُحفظ هذا كله في المتصفح ولا يمسّ الخزانة.
- **بحث في النص الكامل.** مطابقة بالبادئة ومطابقة تقريبية (MiniSearch)، فورية، مع مقتطفات مظلَّلة نُزعت منها علامات ماركداون. ويجيب البحث على [تسميات الوسوم المحلية](arabic-and-rtl.md#localised-tag-labels) كما يجيب على الأسماء الأصلية، و**يطوي الحركات وأشكال الحروف**، فتجد «المقدمة» ملاحظةً تكتبها «الْمُقَدِّمَة»، ويجد `resume` كلمة *résumé*. انظر [البحث بالعربية](arabic-and-rtl.md#searching-in-arabic).
...
- **الشريط الجانبي على أي الجانبين.** ثلاث حالات، في لوحة الأوامر وفي الإعدادات ← المظهر واللغة: *اتبع اللغة* (الافتراضي: يسارًا في الإنجليزية ويمينًا في العربية، ويُعاد النظر فيه كلما تغيرت اللغة)، أو ثبّته على الحافة اليسرى أو اليمنى نهائيًا.
...
- **مراقبة حية للخزانة.** حرّر ملفًا في أي محرر آخر يتحدّث التطبيق خلال نحو 100 مللي ثانية (chokidar + SSE).
...
كل سطر `- [ ]` مهمة. وتُقرأ من آخر السطر الحقول التي علّمت إضافة Tasks الخزائنَ كتابتها: `📅 2026-09-20` موعد الاستحقاق، و`⏳` الجدولة، و`🛫` البدء، و`✅` تاريخ الإنجاز، و`🔁 every week` التكرار، والأولوية `⏫ 🔼 🔽`. ويعدّد سياج ` ```tasks ` المهام من الخزانة كلها مجمّعةً بحسب الملاحظة، وفي المحرر كل مربع فيه حيّ: التعليم يقلب ذلك السطر الواحد في الملاحظة التي يسكنها ويختمه بـ`✅ اليوم`، فتقرأ إضافة Tasks في أوبسيديان الحالة نفسها.
...
تستطيع أن تشير إلى أي فقرة أو بند قائمة. اختمه بمسافة ثم معرّف يبدأ بعلامة الإقحام، مثل `…قاعدة الثلاثة. ^rule3`، أو ضع `^rule3` في سطر وحده تحت الفقرة مباشرة. عندها:

- يربط `[[Note#^rule3]]` إلى تلك الفقرة ويحطّ عليها؛ وتعرض بطاقة التحويم تلك الفقرة وحدها؛
...
تبقى العلامة مخفية في عرض القراءة وفي المعاينة الحية حتى يقف المؤشر على سطرها، فتظهر باهتة بخط ثابت العرض لتُقرأ أو تُغيَّر. ويسكّ **نسخ رابط هذه الفقرة** في لوحة الأوامر معرّفًا للفقرة التي تحت المؤشر (أو يعيد استعمال ما فيها)، ويلحقه بها خطوةَ تراجع واحدة، وينسخ الرابط؛ وعلى سطر عنوان ينسخ رابط القسم بدلًا من ذلك. والصيغة صيغة أوبسيديان، فالخزانة التي تحمل `^ids` أصلًا تحتفظ بكل رابط.
...
يقول الشريط الذي تحت الملاحظة أين أنت، وكم طول الملاحظة، وهل هي منشورة، وفي أي وضع أنت. وينتهي برقم البناء الذي تعمل عليه: [المحدّث](desktop.md#updates) في تطبيق سطح المكتب، وصفحة الإصدارات في المتصفح. أما أدوات الواجهة نفسها فتقف في الأعلى بعد التبويبات: اللوحات، ووضع التركيز، والمخطط، ومصمم الموقع، والإعدادات، والسمة، وتسجيل الخروج.
