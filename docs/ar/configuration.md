...
| `ASTROLABE_DATA` | مجلد بيانات الخادم (الافتراضي `./data`). فيه `settings.json`، وقاعدة بيانات التعليقات (SQLite)، وملفك `custom.css`، و`designs.json`، وملف اعتمادات git، و[رمز القصّاصة](capture.md#القصاصة) (`clip-token`)، ومجلد `fonts/` (ملفات خطوطك، مع نسخة الكتالوج المحفوظة في `fonts/catalog/` والخطوط المرفوعة في `fonts/custom/`) |
...
- **الخزانة**: أين تكتب هذه النسخة الأشياء: مجلد القوالب وقالب الملاحظات الجديدة، و[الملاحظات الدورية](templates-and-notes.md#الملاحظات-الدورية) (صف واحد: المجلد الذي تتقاسمه الأنواع الأربعة، واسم وقالب لكل من اليوم والأسبوع والشهر والسنة)، ومجلد [الملاحظة الفريدة](templates-and-notes.md#الملاحظات-الفريدة) واسمها، ومجلد الرسوم، و**افتح عند التشغيل** (على ماذا يفتح التطبيق: حيث توقفت، أو صفحة السِّجِلّ، أو رفّ المدارات، أو ملاحظة اليوم، أو ملاحظة تختارها؛ فوق الجلسة المستعادة، ولا يعلو أبدًا على رابط ملصوق)، و[صندوق الالتقاط والقصّاصة](capture.md)، و**أين تُكتب المرفقات الجديدة** (انظر [المرفقات](#attachments))، ومجلد الوسوم، وإصدارات الملاحظات، والبحث في PDF.
