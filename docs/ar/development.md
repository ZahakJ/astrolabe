...
| `npm run check-windows-layout` | القشرة عند ستة عروض نافذة × ثلاث نسب بكسل × ثلاثة أوضاع مؤشّر (أدناه) |
...
5. **اسم الموقع الطويل يبقى في لوحته.** يُستبدل الاسم باسم من خمسة وأربعين حرفًا، ويُقاس، ثم يُعاد كما كان؛ وعليه أن ينتهي بثلاث نقاط داخل ترويسة الشريط الجانبي لا أن يتجاوزها.
6. **لا شريط إعادة فتح تحت إصبع.** باب الـ14 بكسل (`.s-reopen`) وسيلة مؤشر، ولا يُرسم حيث يكون بابا اللوحة التمريرة ومفتاحًا بعرض 44 بكسل.

**الوضعان.** `finger` جهاز Pixel 7 بمقاس 390×844. و`stylus` مقاس 720×820 بكثافة 1.5 مع قلم — `availablePointerTypes=6, primaryPointerType=2, availableHoverTypes=3, primaryHoverType=1` — وهو إعداد Blink في متصفح مستقل، لأن `hasTouch` يجعل كروميوم يبلّغ عن جهاز خشن فقط مهما قالت رايات المؤشر. وهو موجود لأن هاتفًا حقيقيًا أجاب `any-pointer: fine` فقُدّم له هيكل سطح المكتب المرسوّ، وبوابة لا تقيس إلا 390 بكسل لا تستطيع رؤية ذلك.

وهي قائمة لأن التدقيق الذي افتتح 3.18 وجد ثلاثة وثلاثين عيبًا من هذه الأشكال بالضبط، وكل واحد منها رقم لم يقسه شيء. والعربية هنا ليست جولة ترجمة، بل تخطيط ثانٍ: ثلاثة من تلك العيوب لم توجد إلا فيها.
