...
same themes, the same visitor scoping, in both languages. And `pdflatex` still typesets it as it
always did, because everything Astrolabe adds is either a LaTeX comment or a macro (a command you
define yourself) that you keep beside the file.
...
- **Links, three ways.** `\note{Fourier Transform}` and `\note[the transform]{Fourier Transform}`
  are Astrolabe's own macro (keep [`astrolabe.sty`](#astrolabesty) beside the file and it typesets
  anywhere). `%% [[Private Scratch]] %%` is a link the PDF never shows. And an existing project's
  links work **unmodified**, because `\input`, `\include`, `\cite`, `\ref` and `\eqref` already say
  what they mean: Astrolabe only looks for what they name in the vault as well, after looking beside
  the file first, so bringing a project into the vault never changes what it produces.
- **Headings and labels are one thing.** A markdown heading and a LaTeX `\label` are the same kind of thing, so
...
The dozen lines that make `\note{…}` understood outside Astrolabe. Download it from your own instance
...
it, `pdflatex` typesets the very same file.
...
| **Ignored** | the set-up at the top of the document (`\documentclass`, `\usepackage`, `\setlength`, `\hypersetup`, spacing commands, `\index`, `\nocite`) — consumed silently, never printed |
...
means nothing in someone else's paper; and BibTeX is not run, so `\cite` looks for a
`\bibitem` in the document or a note carrying that `citekey:`, and is otherwise left as it is.
