...
Open Orbits from the command palette (**Open Orbits**, or **Study due orbits** to go straight into whatever is due), from the ring door in the status bar — a small body on its orbit — or from the line the [Sigils](sigils.md) page shows when cards are due. The address is `/orbits`; the old `/review` still opens it.

The **shelf** lists every deck as a card: its icon, its title and tags, how many are due (counted in orbits: a card coming back around is an orbit closing), new and in all, and a thin line drawn from the last thirty days of your grades. The header counts what is due today across all of them and how many days in a row you have studied. **Study** starts a session with the due cards and today's new ones; **Study section ▾** limits it to one heading. *Everything else* sits last.
