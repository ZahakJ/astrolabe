...
/** ASTROLABE_DATA/fonts/catalog — everything this module writes lives under it. */
export function catalogDir(): string {
  return path.join(fontsDir(), "catalog");
}

/** ASTROLABE_DATA/fonts/catalog/<id> for a KNOWN catalog id. The id is checked
...
export interface FontCssOptions {
  /** Family-name prefix: "Astrolabe" for the live site, "AstrolabePreview" for the
...
export async function buildFontCss(slots: FontSlots, opts: FontCssOptions): Promise<string> {
  const parts: string[] = [
    "/* Generated by Astrolabe — self-hosted webfaces from ASTROLABE_DATA/fonts/catalog.",
  ...
  if (opts.root && vars.length > 0) {
    parts.push(
      ...
      "   this wins; and a ASTROLABE_DATA/custom.css :root rule (linked after this",
...
/** ONE face per font id, under its own `AstrolabeOpt-…` family — what makes the
...
export async function buildFaceListCss(ids: string[]): Promise<string> {
  const parts: string[] = [
    "/* Generated by Astrolabe — one face per pickable font, for the settings",
