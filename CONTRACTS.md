...
- **A post's fallback date is the ledger's, not the inode's** (`server/created.ts`, `VELLUM_DATA/created.json`): saves rename a temp file over the note and so mint a new birthtime on every edit; the indexer records the first birthtime it sees per path (seeded, once, from `git log --diff-filter=A` when the vault is a repository, taking the earlier) and answers with it until the path is forgotten. Frontmatter `date`/`created`/`published` still wins.
