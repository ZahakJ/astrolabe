...
%---%
%
% This block is Astrolabe's frontmatter. Both fences are LaTeX COMMENTS, so
% pdflatex never sees it — which is the deal this whole file is built on:
% everything Astrolabe adds must be invisible to the other toolchain.

\documentclass{article}
\usepackage{astrolabe}   % get it from your own instance: /api/astrolabe.sty
\usepackage{amsmath}
\usepackage{graphicx}

\newcommand{\R}{\mathbb{R}}

\title{LaTeX in Astrolabe}
\author{The vault}
...
\texttt{\textbackslash ref} and \texttt{\textbackslash eqref} already say what they mean, so Astrolabe
