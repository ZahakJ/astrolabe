...
had something to show. The real point of Astrolabe is to sit on top of
...
Astrolabe never converts, moves, or wraps your files. It reads markdown, it
...
Astrolabe is a single Node process (Node ≥ 24) listening on port `6801`
(`PORT` overrides). A systemd unit is all it takes on a home server:

```ini
[Service]
Environment=ASTROLABE_VAULT=/srv/notes
ExecStart=/usr/bin/npm start --prefix /opt/astrolabe
Restart=on-failure
```

Put a reverse proxy with auth in front if the machine is reachable from the
internet — Astrolabe itself trusts everyone who can reach the port, on the
