...
import { FEEDS_TAB, isBookPath, isFeedsTab, isMediaTab, isOrbitsTab, isRoutinesTab, MEDIA_TAB, orbitsSessionOf, ORBITS_TAB, ROUTINES_TAB } from "../workspace.ts";
import type { Screen } from "./nav.ts";

const LIST_SURFACES = (tab: string): boolean =>
  tab === "~library" || isRoutinesTab(tab) || isMediaTab(tab) || isFeedsTab(tab) || (isOrbitsTab(tab) && orbitsSessionOf(tab) === null);
