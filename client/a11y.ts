...
export interface DialogOptions {
  ...
  active?: boolean;
  /** Keep the TRAP but leave restoration to the caller. For a surface whose
   *  close can also NAVIGATE — the blog's search overlay picks a result and
   *  the page under it changes — "focus whatever opened me" is the wrong
   *  answer half the time, and the surface already knows which half it is in.
   *  Default true; set false only when something else restores. */
...
export function useDialog(
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (opts.current.restoreFocus === false) return;
