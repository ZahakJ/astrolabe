...
export type PaneSurface = "edit" | "reading" | "book" | "drawing" | "graph" | "media" | "routines" | "orbits" | "review-week" | "calendar" | "today" | "timeline" | "library" | "empty";
...
export function isVirtualTab(path: string): boolean {
  return isGraphTab(path) || isMediaTab(path) || isRoutinesTab(path) || isOrbitsTab(path) || isReviewWeekTab(path) || isCalendarTab(path) || isTodayTab(path) || isTimelineTab(path);
}
