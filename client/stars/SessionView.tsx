...
import { EVERYTHING_ELSE, type ConstellationMeta, type Star } from "../../shared/constellations.ts";
...
import { tickSlotForConstellation } from "../routines/stars.ts";
import { useStore } from "../state.ts";
import { toast } from "../toast.ts";
import { starsTabFor } from "../workspace.ts";
import Face from "./Face.tsx";
import { headOf, StarQueue, type Pick, type Preview } from "./queue.ts";
...
function previewText(p: Preview): string {
  if ("days" in p) return stf("starsInDays", { n: localeNum(p.days) });
  if (p.minutes >= 60 * 24) return stf("starsInDays", { n: localeNum(Math.round(p.minutes / (60 * 24))) });
  if (p.minutes >= 60) return stf("starsInHours", { n: localeNum(Math.round(p.minutes / 60)) });
  return stf("starsInMinutes", { n: localeNum(p.minutes) });
}
...
function kindLabel(p: Pick): string {
  if (p.phase === "new") return st("starsKindNew");
  if (p.phase === "review") return st("starsKindReview");
  return p.phase === "relearning" ? st("starsKindRelearn") : st("starsKindLearning");
}
...
export default function SessionView({ path, section }: { path: string; section: string | null }) {
  ...
  // constellation and nothing is due here any more, the slot is ticked for
  // today through client/routines' helper — an effect and not part of the
  // render, because a tick is a write. The implicit constellation is no
  ...
  useEffect(() => {
    ...
    void tickSlotForConstellation(path);
