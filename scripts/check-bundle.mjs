...
// 3.30.1: THE SWEEP MERGED OVER 3.30.0 (webmentions and the fediverse). The
// sweep's numbers were measured before that round landed; its 41 keys and
// four count units now sit in the language chunks (the larger, Arabic, is
// what every audience counts) and its shell bits in the entry: entry 744.6 →
...
// 3.30.2: the "server moved on" notice learned to tell a deploy from a stale
...
// READ ALOUD: 862.8 → 867.9 kB, +5.1 — the entry's +4.3 and the phone
// shell's player footing in phone.css and its lazy SpeechLayer boundary.
// 863 → 868.
...
// …and for 3.5.0 (653.7 kB actual → 660): the preferences that travel with
  ...
  // READ ALOUD: 753.8 → 758.1 kB, +4.3, measured against a build of 3.31.1
  // (6dfd491). The Arabic dictionary is 3.2 kB of it (the player, the chip,
  // the Settings row and its install phases, the device-voice notes — every
  // audience counts the larger language); the rest is `api.speakAudio` /
  ...
  // YOUR OWN VOICES: 762.2 → 768.6 kB, +6.4, measured against a build of
  // 3.35.1 (3070f5f). The Arabic dictionary is +4.9 kB of it (the row, its
  // ⓘ, the scan's reasons, the path refusals, the external speaker's note,
  // the pickers' groups — the larger language counts), the stylesheet +1.3
  // (the row's layout and the pickers' phone rules; settings.css is in the
