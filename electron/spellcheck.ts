...
//      answers (he, fa, ar, fr, ja, or null → inherit the instance's own), so
//      those plus the instance language are what gets enabled. Enabling a
//      language Chromium has no dictionary for throws, so each is tried
//      against the platform's own list first. French is the one Latin
//      answer: a line the editor corrects as French (shared/french.ts) is
//      checked against the French dictionary, not underlined by the
//      English one for every word it just fixed. Japanese has no dictionary
//      anywhere and is simply dropped, which is the honest outcome: the
//      browser then leaves those lines alone instead of underlining every
//      word of them.
//
...
export const LINE_LANGUAGES = ["he", "fa", "ar", "fr", "ja"] as const;
