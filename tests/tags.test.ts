...
import { initIndexer, tags as tagsRaw } from "../server/indexer.ts";
...
// `tags()` is scoped by audience AND by language (a visitor's tag list drops
// the tags only language-hidden notes carry). These cases are about counting
// and normalization, so the language scope is pinned to `null` — unfiltered.
const tags = (publishedOnly = false) => tagsRaw(publishedOnly, null);
