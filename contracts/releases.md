...
- **3.33.2** — the desktop package ships no graphics-card build of whisper.cpp: transcription there is the processor path, and the download is about 50 MB smaller ([sync-desktop-mobile.md](sync-desktop-mobile.md)).
- **3.34.0** — **Back means up** (a reader on a Galaxy Z Fold): a folder's ‹ goes to its parent by path, a tab switch pushes an entry per level, and the phone's stack survives a reload in sessionStorage; a folder's path as crumbs; the Notes tab as a Tree or as Folders, a folder's files one folded row, an empty folder with New note here, pull down to refresh (the pocket pulls); two columns by the screen's shape (the open Fold), with a grip, a remembered width and the note's sheet over its column; the pocket's "synced" line hidden as it always meant to be ([shell.md](shell.md), "The phone shell").
