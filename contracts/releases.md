...
- **Unreleased** — the sweep, part 1: the contracts split into `contracts/`; one phone width in every stylesheet; the rules the phone and the server shared by copy moved into `shared/` with a parity test; Sigils by their own name in the code; the copy scan on the syntax tree, the Android shell's words held to parity; budgets for the Sigils and Calendar pages ([gates.md](gates.md)).
