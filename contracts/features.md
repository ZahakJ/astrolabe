...
*Self-contained: `shared/speech.ts` (every rule, pure), `server/speak.ts` (the door), `server/speakEngine.ts` + `server/speakWorker.py` (the venv, the models, the persistent child), `server/speakQueue.ts` (one at a time), `server/speakCache.ts` (the LRU), `server/standalonePython.ts` (the Python fetched for a machine with none), `client/speech/*` (the player, the chip, the doors, `deviceVoices.ts`), `client/components/settings/ReadAloudControls.tsx` (the row's controls), `electron/systemVoice.ts` (Windows' chosen voice). Docs: `docs/read-aloud.md` and its Arabic twin. Tests: `tests/speech.test.ts`, `tests/speakCache.test.ts`, `tests/speakQueue.test.ts`, `tests/speakRoutes.test.ts`, `tests/standalonePython.test.ts`, `tests/deviceVoices.test.ts`.*
