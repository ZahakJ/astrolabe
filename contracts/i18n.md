...
- **A directional *glyph* is mirrored ONCE — and by hand where it matters.** `›` (U+203A),
  ...
  breadcrumb, the sidebar month's and the Calendar page's prev/next, the properties shelf's
  chevron, the sidebar tree's chevrons (`.s-tree__chevron`, which every collapsible panel
  header borrows), the tag tree's branch chevrons, and the status bar's, the Library's and the
  designer's breadcrumb separators are therefore PINNED — `direction: ltr; unicode-bidi:
  isolate` on the glyph's own element (`inline-block` where it was inline, so it can take a
  transform), so nothing mirrors it — and flipped under `[dir="rtl"]` with `scaleX(-1)`; an
  OPEN disclosure chevron is `rotate(90deg)` in both directions. `tests/rtlGlyphs.test.ts`
  finds every element whose whole text is one such glyph (a literal or template-literal class
  list), refuses one that is neither guarded nor knowingly left, and refuses a hand flip on a
  glyph that is not pinned. Knowingly left: the selection menu's submenu chevron. SVG paths are geometry, not
...
**THE WAY BACK IS ALWAYS ON SCREEN (`client/chromeLangSwitch.ts`, `langPref.ts::chromeLangPref`, the store's
`toggleChromeLang`).** A palette row still needs a reader who knows the chord that opens the
palette, and Settings → This device → Editor language is a row in a panel written in the language
they cannot read. So an admin session carries one action with five faces, and every face goes
through `toggleChromeLang()` so none can disagree about where a press lands:

| face | where | shell |
| --- | --- | --- |
| the language key (`data-testid="chrome-lang"`) | the status bar's far end, beside the mode pills | desktop |
| the same key (`client/phone/LangPill.tsx`) | the top bar of **More** and of **Settings** | phone |
| a row labelled in both languages | the note screen's ⋯ sheet, Actions | phone |
| `Ctrl/Cmd Alt Shift L` (`scChromeLang`) | anywhere, **modals included**, admin | both |
| the palette row `chrome-lang-toggle` | `Ctrl/Cmd P`, or the phone's Search | both |

- **The label is the OTHER language, in its own script**: `ع` while the chrome is English, `EN`
  while it is Arabic. Tooltip and accessible name are both sentences, the one on screen first
  ("Switch to Arabic · التبديل إلى العربية"). The palette row leads with the TARGET language
  ("التبديل إلى العربية · Switch to Arabic" from English) and answers to `CHROME_LANG_ALIASES`
  (English, Arabic, الإنجليزية, إنجليزي, العربية, عربي, …) through the palette's alias haystack
  (`client/paletteRank.ts::rankCommands`, ranked as a hint hit), so "english", "arabic",
  "إنجليزي" and "عربي" all find it whichever way it points.
- **Those sentences are NOT in the dictionary**, and that is the one exception to "every string
  through t()": a page holds one dictionary (the other is a chunk loaded on demand), and the half
  of the sentence this control needs most is the half in the language that is not loaded.
- **It writes the editor-language preference**, never the visitor's: the other language, landing
  on `null` (follow the site) when that is the site's own — the rule `setVisitorLang` already uses
  for a signed-in owner — so two presses come home to the default rather than leaving a pin.
  The Settings row, the peer windows (the bus carries `editorLang`) and the switch's wait for its
  dictionary chunk all follow from `setEditorLang`, unchanged.
- **It is not the twin pill.** `EN ⇄ ع` turns a NOTE to its other face and sits with the note's
  facts; this key is bordered, carries a globe, and sits at the other end with the switches. It
  holds its mousedown so the editor keeps focus and caret.
- Visitors keep the public site's `LangSwitch`; none of the five faces render or answer for them.
- Held by `tests/langPref.test.ts` (the label rule, the preference it writes, two presses home),
  `tests/paletteRank.test.ts` (the four searches), `tests/keymap.test.ts` (the chord's ledger row),
  check-fidelity (the desktop key flips `<html lang/dir>` and back) and check-phone (More's key
  and the note sheet's row).
...
**The pocket speaks the reader's language, not the phone's.** A pocket vault's refusals — every
`501 code:"pocket"` reason, a refused settings key, a path `vaultIo.ts` will not touch (a `400`) —
are KEYS into the shell's dictionary (`mobile/src/i18n.ts`, `refuse*`, `keep*`, `vaultPath*`),
spoken at the moment of the answer in the client's chrome language: the pocket shares the page with
the client and reads its `<html lang>` (`readerLang()` in `pocket/server.ts`; the shell's own pick
from the phone's languages before the client has booted). The client's path is unchanged — the
body's `error` becomes the `ApiError`'s message and is shown as it came — so an Arabic reader on an
English phone reads Arabic. The scan refuses a sentence typed as a `fail(501, …)` reason or as a
`PocketVaultError` argument (`scripts/i18nScan.mjs`); `tests/pocketWords.test.ts` holds each way a
refusal is reached to both languages.
