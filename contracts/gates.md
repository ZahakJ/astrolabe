...
| `check-perf.mjs` | On a generated 2,000-note vault at 4× CPU, best of several rounds: first paint, keypress → paint (median, p95), long-task time, reading render, the Sigils and Calendar pages drawn, and the phone's Tree opening a folder of fifty with no long task — each under its budget. |
| `check-phone.mjs` | The phone shell driven like a phone on six shapes in both languages (a Galaxy Z Fold's cover and open screens among them): a tree tap changes the URL, Back pops screens and closes sheets first, a folder's ‹ goes up after a reload and after another tab, the crumbs go up, the Tree opens and remembers, two columns keep the list's place, Publish asks, a long press is a menu; every screen and sheet (the Outline and Backlinks panes with their rows) has 44px targets, 16px fields, nothing sideways and nothing covered. |
