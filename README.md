...
- **[Flashcards](docs/flashcards.md)** — every `==highlight==`, `> [!quote]` and `Question / ? / Answer` block is already a card; the Review page asks them back on the SM-2 schedule the Obsidian Spaced Repetition plugin uses, and writes the due day into the note as that plugin's own comment
