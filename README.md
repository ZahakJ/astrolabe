...
- **[Routines](docs/routines.md)** — a `routine` fence is a plan by the day (an exercise week with morning and evening columns, the five prayers, sleep, water) and the `routine-log` the app writes under it is the record; the card shows today's checklist, the streak, the week and a twelve-week heatmap, and the Routines page asks you every morning, with templates built in and yours saved beside them
