// THE TWENTY HAND-DRAWN FOLDER GLYPHS — the original set, kept verbatim.
//
// Drawn for this app before the catalog grew (shared/folderIconCatalog.ts);
// the generator (scripts/gen-folder-icons.mjs) copies them into
// folderIconPaths.ts beside the Lucide drawings. They stay in their own file
// so a hand-tuned path is never overwritten by a generator run, and so the
// house rules they were drawn under stay attached to the drawings:
//
...
//  * Every glyph must survive 14px.
//  * NO TWO CONFUSABLE at 14px, including against the attachment glyphs that
//    share the tree: `music` is a SINGLE note because the audio attachment
//    glyph is the beamed pair, and `book` is the OPEN book because a closed
//    one reads as `archive`.
...
export const FOLDER_ICON_HAND_PATHS: Record<string, string[]> = {
