...
import { dayRatio, dayStatus, tasksFor, type DayStatus, type RoutineEntry, type RoutinePlan } from "./routine.ts";
...
export function emptyAgenda(iso: string): DayAgenda {
  return { iso, note: null, sigils: [], decks: [], trackers: [], count: 0 };
}
...
export function agendaByDay(days: readonly string[], sources: AgendaSources, today: string): Map<string, DayAgenda> {
  ...
  for (const day of out.values()) {
    ...
    day.count = (day.note === null ? 0 : 1) + day.sigils.length + day.decks.length + day.trackers.length;
  }
  return out;
}
